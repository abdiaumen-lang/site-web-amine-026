        ...
        async function fetchCategories() {
            try {
                const { data, error } = await supabase
                    ...
                    .limit(6);
                if (data && !error && data.length > 0) setCategories(data);
            } catch (err) {
        ...
        <>
            ...
            <div className="w-full bg-[#FF6600] text-white text-xs font-bold py-2 overflow-hidden whitespace-nowrap">
                <div className="animate-marquee-smooth">
                    <span className="px-8">{settings?.top_bar_text || "🚀 Livraison offerte à Alger à partir de 50 000 DA ! | 📞 Contactez-nous : +213 555 123 456 | 🏷️ Offres spéciales sur l'électroménager cette semaine !"}</span>
                    <span className="px-8">{settings?.top_bar_text || "🚀 Livraison offerte à Alger à partir de 50 000 DA ! | 📞 Contactez-nous : +213 555 123 456 | 🏷️ Offres spéciales sur l'électroménager cette semaine !"}</span>
                </div>
            </div>
                    ...
                    <nav className="hidden md:flex items-center justify-center gap-8 mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                        ...
                        {categories.map((cat) => (
                            <div key={cat.id} className="relative group">
                                ...
                                >
                                    {cat.name}
                                    {cat.subcategories?.length > 0 && <span className="material-symbols-outlined text-[16px]">expand_more</span>}
                                </Link>
                                    ...
                                    <div className="absolute top-full left-1/2 -translate-x-1/2 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                                        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 py-2 w-48 flex flex-col">
                                            {cat.subcategories.map((sub: any) => (
                                                <Link key={sub.id} href={`/product?category=${cat.id}&subcategory=${sub.id}`} className="px-4 py-2 text-sm text-slate-600 dark:text-slate-400 hover:text-primary hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                                                    {sub.name}
                                                </Link>
                                            ))}
                                        </div>
            ...
            {/* Mobile Search — full-screen portal */}
