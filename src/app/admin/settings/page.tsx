                                ...
                                <div className="pt-8 border-t border-slate-200 dark:border-slate-700">
                                    <div className="flex items-center justify-between mb-6">
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500 mt-1">Synchronisez automatiquement vos commandes et les informations de vos clients dans une feuille de calcul Google.</p>
                                        </div>
                                    </div>

                                    <div className="space-y-5">
                                        <div>
                                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Google Apps Script Webhook URL</label>
                                            <input
                                                type="url"
                                                value={googleSheetsWebhookUrl}
                                                onChange={(e) => setGoogleSheetsWebhookUrl(e.target.value)}
                                                placeholder="https://script.google.com/macros/s/.../exec"
                                                className="w-full rounded-xl border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-4 py-2.5 text-sm focus:ring-green-500 focus:border-green-500 font-mono"
                                            />
                                        </div>

                                        <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl">
                                            <div className="text-xs text-green-800 dark:text-green-300 flex items-start gap-2">
                                                <span className="material-symbols-outlined text-sm shrink-0">help</span>
                                                <span>
                                                    Pour utiliser cette fonctionnalité, vous devez créer un script dans votre Google Sheet et le déployer en tant qu'application Web accessible à "Tout le monde". <button type="button" className="underline font-bold" onClick={() => {
